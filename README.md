...
- Annullamento prima dell’inizio dell’invio; stati distinti per programmazione, invio affidato a WhatsApp, consegna/lettura, mancato invio ed esito incerto. Nessun retry automatico dei tentativi ambigui; un cambio dell’account WhatsApp sul PC blocca i messaggi programmati dal precedente account. Le richieste duplicate riutilizzano un identificativo persistente.
...
Verifica locale della 0.5.0: **11 test del componente** con adapter fittizio e HTTP locale, **38 controlli Swift** su configurazione, numeri, date e bozze precedenti e **build iOS Simulator riuscita**. Anche la build firmata iPhone è riuscita; la versione 0.5.0 build 7 è stata installata e avviata sul telefono collegato. Il [pacchetto Windows definitivo](https://github.com/Domenico-lel/iDom/actions/runs/33961249763) ha superato test Windows/Linux, sintassi PowerShell 5.1, avvio Edge su pagina vuota e avvio del servizio confezionato in simulazione. La [verifica UI finale](https://github.com/Domenico-lel/iDom/actions/runs/33961011357) ha superato tutti e 5 i test, compresa la configurazione WhatsApp separata da PC Remote. Il primo invio reale richiede ancora il QR dell’utente sul PC e un messaggio di prova al proprio numero. Non è stato inviato alcun messaggio reale dagli strumenti di sviluppo.


### Correzione collegamento WhatsApp · 0.5.1

Build 9: l’icona del modulo Messaggi usa il simbolo di sistema `bubble.left.and.bubble.right.fill`, condiviso da Home e Tools. Il precedente nome `message.badge.clock.fill` lasciava il riquadro vuoto sull’iPhone dell’utente. Il controllo del catalogo di sistema locale conferma che il nome precedente non viene risolto e quello nuovo è disponibile. Build firmata riuscita e aggiornamento installato sull’iPhone. La correzione riguarda soltanto l’icona e non modifica il collegamento o la coda WhatsApp.

Durante il collegamento, soltanto la finestra di configurazione presenta l’errore: la schermata principale non apre più lo stesso avviso contemporaneamente. Un errore 502 durante la verifica spiega che occorre controllare il componente locale e la destinazione Tailscale Serve sulla porta 47322. Questa modifica non ripara da sola un componente Windows fermo o una destinazione proxy errata. Le richieste di invio con risposta 502 restano incerte e conservano lo stesso identificativo, senza reinvio automatico. Nessuna modifica a chiavi, sessioni o messaggi programmati.

Verifiche 0.5.1 build 8: **40 controlli Swift superati**, inclusi 502 nella lettura dello stato e conservazione dell’incertezza dopo un invio, e **build firmata iPhone riuscita e installata sul telefono**. La verifica sul telefono dell’avviso singolo resta da completare. Durante la diagnosi sul PC dell’utente, Tailscale Serve risultava correttamente configurato sulla porta 8444 verso 127.0.0.1:47322. Dopo l’avvio del componente, lo screenshot dell’utente conferma che iDom legge lo stato `pairing` e il 502 non compare più: il collegamento HTTPS autenticato funziona, mentre la sessione WhatsApp richiede ancora la scansione del QR. Primo invio reale ancora da verificare.
