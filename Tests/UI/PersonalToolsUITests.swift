...
final class PersonalToolsUITests: XCTestCase {
    ...
    func testPCRemoteRequiresPairingBeforePowerCommands() {
        ...
        app.textFields["pc.address"].tap()
        app.textFields["pc.address"].typeText("http://example.org")
        XCTAssertFalse(app.buttons["Salva"].isEnabled)
        app.buttons["Annulla"].tap()
        app.buttons["pc.configure"].tap()
        app.textFields["pc.address"].tap()
        app.textFields["pc.address"].typeText("https://pc-idom-ui-test.invalid.ts.net:8443")
        app.secureTextFields["pc.token"].tap()
        app.secureTextFields["pc.token"].typeText(String(repeating: "a", count: 64))
        XCTAssertTrue(app.buttons["Salva"].isEnabled)
        capture("PC Remote · configurazione")
        app.buttons["Salva"].tap()
        XCTAssertTrue(app.buttons["Modifica collegamento"].waitForExistence(timeout: 15))
        app.terminate()
        app.launch()
        openTool("PC Remote")
        XCTAssertTrue(app.buttons["Modifica collegamento"].waitForExistence(timeout: 15))
        XCTAssertFalse(app.buttons["Accendi"].isEnabled)
        XCTAssertFalse(app.buttons["Spegni"].isEnabled)
        let remove = app.buttons["Rimuovi collegamento"]
        app.swipeUp()
        let ready = expectation(for: NSPredicate(format: "enabled == true"), evaluatedWith: remove)
